  ...
  return (
    <MathJaxContext config={mathJaxConfig}>
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8 min-h-screen">
              ...
              <div className="relative" ref={uploadMenuRef}>
                ...
                {uploadMenuOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded shadow-lg z-10 flex flex-col">
                    ...
                    <button
                      className="px-4 py-2 text-left hover:bg-gray-100"
                      onClick={() => { setUploadMenuOpen(false); setScanModalOpen(true); }}
                    >
                      Upload by Scan
                    </button>
                  </div>
        ...
        {/* Scan Upload Modal */}
        <Modal isOpen={scanModalOpen} onClose={() => setScanModalOpen(false)} title="Upload Questions by Scan">
          <form
            onSubmit={async (e) => {
              e.preventDefault();
              if (!scanFile) return;
              ...
              try {
                const formData = new FormData();
                formData.append('pdfFile', scanFile);
                const response = await questionService.getOpenAiPdf(formData);
                const extractedText = response.data.text;
                // Re-fetch questions after successful upload
                const responseQuestions = await questionService.getAllQuestions();
                setQuestions(responseQuestions.data);
                setScanModalOpen(false);
                setScanFile(null);
                setToastType('success');
                setToastMessage('Questions scanned and uploaded successfully!');
              } catch (err) {
                setToastType('error');
                setToastMessage('Failed to scan and upload questions.');
              } finally {
          ...
          >
            <div className="mb-4">
              <div className="mb-2 text-sm text-gray-700">
                Upload an image or PDF of your exercise. Our AI will scan and create questions automatically.
              </div>
              <input
                type="file"
