...
const AttemptDetails: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAttemptDetails = async () => {
      ...
      try {
        ...
        if (res.data.testId && Array.isArray(res.data.testId.questions) && res.data.answers) {
          ...
          const questionsWithAnswers = res.data.testId.questions.map((q: Question) => {
            ...
            return {
              ...q,
              userAnswer,
              isCorrect: (q.correctAnswer as string[]).map(correctAnswer => correctAnswer.toLowerCase())
  ...
  if (!attempt || !questions.length) {
    return <div className="min-h-screen flex items-center justify-center text-red-600">Failed to load attempt details.</div>;
  }
